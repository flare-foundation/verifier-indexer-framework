...
- `db_name` and `username` are required: an empty `db_name` let the server choose the database (the role's own, or `PGDATABASE`), so the framework migrated into and indexed a database nobody reads, while the health endpoint answered ready. Environment overrides apply before validation, so `DB_NAME` and `DB_USERNAME` still satisfy it.
- Startup warns when `end_block_number` lies below the resume point: nothing in the configured window is indexed, and the warning names the re-index procedure. `start_block_number` set across an already-indexed range silently repaired nothing, which is what an operator reached for.
...
- When `history_drop` is enabled, an entity in `HistoryDropOrder` with a soft-delete field (`gorm.DeletedAt`) is rejected at startup. gorm rewrites a delete on such an entity into `UPDATE … SET deleted_at`, so the drop reported success, advanced the first-indexed boundary and kept every row. No shipped consumer declares one.
...
- `end_block_number` bounds every fetched range. It was only checked after an iteration completed, so a bounded run indexed up to `max_block_range - 1` blocks past its end and left `last_indexed_block_number` beyond it.
